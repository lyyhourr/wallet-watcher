...
export default function AuthLayout(props: IAuthLayout) {
    return (
        <main className='h-screen grid lg:grid-cols-5 overflow-hidden'>
            ...
            <section className='w-full h-full border-2 border-black lg:col-span-3  '>
                <div className='flex flex-col justify-between h-full px-5 py-8'>
                    <Link className='text-black flex text-lg items-center  group' href={props.route.href}>
                    ...
                    <div className='flex flex-col gap-10 items-center  justify-center'>
                        <h1 className={`${fontHeader.className} text-7xl tracking-wide`}>{props.title}</h1>
                        <h5 className={`${fontHeader.className} text-xl `}>{props.subTitle}</h5>
                        {props.children}
                    </div>
                    <div className='text-sm sm:text-base md:w-[550px] mx-auto'>
                        <p className='text-gray-600  '>By {props.footerTitle}, I have read and I understand and agree to, The [web name]
                            {" "}<span className='text-black underline'>Term of Use</span>{" "}
                            and
