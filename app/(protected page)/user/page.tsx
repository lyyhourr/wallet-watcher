...
import ResetData from "@/components/protected_pages/user/Reset-Gmail";
...
import ChangeGmail from "@/components/protected_pages/user/Reset-Gmail";
...
const FetchData = async () => {
  ...
  const { username, gender, dob, phonenumber, country, city, about } =
    data?.length && data[0];
...
export default async function UserInfo() {
  ...
  const phoneNumber = phonenumber?.length
    ? phonenumber
      ...
      <main className="w-full flex gap-2 py-3 flex-col xl:flex-row">
        <section className="  w-full h-full flex flex-col gap-4 px-3 py-2">
          <div className="bg-slate-50 rounded-2xl border-slate-200 border-2 flex flex-col gap-6 p-5">
            ...
            <div className="flex w-full justify-between">
              <p className={cn(!dob?.length && "text-gray-600")}>
                {dateOfBirth} {dob?.length && `( dob )`}
              </p>
              <UpdateUser
            ...
            <div className="flex w-full justify-between">
              <p className={cn(!phonenumber?.length && "text-gray-500 ")}>
                {phoneNumber}
              </p>
              <UpdateUser
        ...
        <section className=" w-full h-full flex flex-col gap-4 px-3 py-2">
              ...
              <div className="flex w-full justify-between">
                <div className="flex gap-2">
                  {country?.length ? (
                    ...
                    <p className={cn(!country?.length && "text-gray-600")}>
                      {Country}
                    </p>
                  )}
                </div>
              ...
              <div className="flex w-full justify-between">
                <div className="flex gap-2">
                  {city?.length ? (
                    ...
                    <p className={cn(!city?.length && "text-gray-600")}>
                      {City}
                    </p>
                  )}
                </div>
          ...
          <div className="bg-red-100 rounded-2xl border-red-400 border-2 h-full flex flex-col gap-6 p-5">
            <div className="flex w-full flex-col gap-5">
              <p className={`${inter.className} text-xl text-red-600`}>
                Danger Zone
              </p>
              <div className="flex gap-3 flex-col sm:flex-row sm:items-center sm:justify-around">
                <Password />
                <ChangeGmail />
                <DeleteAccount userId={user ? user.id : ""} />
              </div>
            </div>
          </div>
        </section>
