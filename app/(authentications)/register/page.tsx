...
export default function Register() {
    ...
    const validation = () => {
        if (!formData.username.length || !formData.email.length || !formData.password.length || !formData.cfpassword.length) {
            ...
            setMessage({ color: "red", msg: "username must be more than 3 characters" })
            return false
        }
        else if (formData.password.length < 5) {
            setMessage({ color: "red", msg: "Password must be more than 5 characters" })
            return false
    ...
    const handleRegister = async (e: any) => {
        ...
        const { data: { user: { identities } } }: any = res;
    ...
    >
        <form action="" className='flex flex-col gap-1 md:gap-7' onSubmit={handleRegister} method='post'>
            <Input placeholder='Username' className='p-7 text-lg' name='username' onChange={handleChange} value={formData.username} />
            <Input placeholder='Email..' className='p-7 text-lg' name='email' type='email' onChange={handleChange} value={formData.email} />
            ...
            <Button disabled={isLoading} className={cn(`bg-primary-color text-lg mt-2 sm:mt-0`, isLoading && "animate-pulse")}>{isLoading ? "Registering" : "Register"}</Button>
        </form>
        <div className='flex gap-2 justify-center items-center'>
            <p>Already Have an account?</p>
            <Link href={"/dashboard"} className='text-primary-color underline'> Login Now </Link>
        </div>
        <p className={cn('px-3 py-5 w-full text-center  underline',
